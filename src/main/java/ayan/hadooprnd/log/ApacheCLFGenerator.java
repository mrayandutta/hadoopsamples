...
/*
 * Dummy Log File Generator
 */
public class ApacheCLFGenerator 
{
    static String fileName ="webLog.txt"; 
    ...
    public static void writeApacheLogs(int recordCount) throws IOException 
    {
        FileWriter fwriter = new FileWriter(fileName);
        BufferedWriter bwriter = new BufferedWriter(fwriter);
        List<String> ips = generateIps(4, recordCount);
        List<String> uas = generateUas(recordCount);
        for (int i=0; i < recordCount; i++) 
        {
            String line = String.format("%s %s \"%s\" 200 140 \"%s\"", ips.get(i), generateTimestamp(), pages[i % (r.nextInt(pages.length) + 1)], uas.get(i));
